...
class ViewPyGame:
    '''Class who will handle the creation and filling
    of the window using Pygame'''
    def __init__(self):
        pygame.init()
        self.size = WIDTH , (HEIGHT + INVENTORY_HEIGHT)
        self.screen = pygame.display.set_mode(self.size)
        '''Dict containing pairs of data/images representing
        the diferents type of tile in the maze'''
        self.images = {
    ...
    def load_images(self):
        '''Method who load the images of the dict 'self.images'
    ...
    def resize_images(self):
        '''Method that resize every loaded images to
        CASE_SIZEpx * CASE_SIZEpx'''
        for key in self.images:
            if key == 'inventory':
                self.images[key] = pygame.transform.scale           \
                (self.images[key],(CASE_SIZE * 15, INVENTORY_HEIGHT))
            else :
                self.images[key] = pygame.transform.scale           \
                (self.images[key],(CASE_SIZE, CASE_SIZE))
    def show_maze(self, map):
        '''Method that put initial map on the window Surface'''
        for y in range(0,15):
            for x in range(0,15):
                self.screen.blit                                    \
                (self.images[map[y][x]], (x*CASE_SIZE, y*CASE_SIZE))
        self.show_inventory()
        pygame.display.flip()
    def show_inventory(self):
        '''method who put the inventory bar on the screen,
        used both a the start of the game and when the 3 items are found'''
        self.screen.blit((self.images['inventory']),                    \
        (0, 15 * CASE_SIZE,15 * CASE_SIZE, INVENTORY_HEIGHT))
    def add_items_to_inventory(self, char, index_of_item):
        '''Method who add to the inventory the image
        of the item the hero just picked up'''
        self.screen.blit(self.images[char],                            \
        ((index_of_item * CASE_SIZE) + 100, 15.1 * CASE_SIZE))
        pygame.display.flip()
    def update_hero_position(self, oldpos, newpos):
        ...
        self.screen.blit                                               \
        (self.images[PATH_CHAR],( ox * CASE_SIZE, oy * CASE_SIZE))
        '''Before puting the Hero image in his new position we need
        to fill the zone with black'''
        pygame.draw.rect(self.screen, (0,0,0),                          \
        (nx * CASE_SIZE, ny * CASE_SIZE, CASE_SIZE, CASE_SIZE))
        self.screen.blit(self.images[HERO_CHAR],                        \
        ( nx * CASE_SIZE, ny * CASE_SIZE))
        '''Updating only the two rectangles we changed'''
        pygame.display.update                                           \
        (((ox * CASE_SIZE, oy * CASE_SIZE, CASE_SIZE, CASE_SIZE),
        (nx * CASE_SIZE, ny * CASE_SIZE, CASE_SIZE, CASE_SIZE)))
    ...
    def end_screen_event(self):
        '''Method who wait for either 'y' or 'n' key to be pressed and
